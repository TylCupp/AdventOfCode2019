...
function findWirePoints(directions) {
  ...
  for (let i in directions) {
    ...
    for (let j = 0; j < steps; j++) {
      ...
      if (points[key] == null) points[key] = step;
...
let intersection = intersections.reduce(
  (accumulator, currentValue) =>
    mDistance(accumulator) > mDistance(currentValue) ? currentValue : accumulator,
  intersections[0],
);

console.log(intersections);
console.log('Part One:', mDistance(intersection));
console.log('Part Two:', intersections[0][2], intersections);
